...
void hanoi_iteratif(int n, char source, char target, char auxiliary) {
    ...
    int moves = 0; // Compteur de mouvements
...
void add_log(struct Log logs[], char algo[], int n, double execution_time, int* log_count) {
...
void display_logs(struct Log logs[], int log_count) {
    ...
    for (int i = 0; i < log_count; i++) {
        printf("%d - %s | %d disques | Temps d'ex%ccution : %f secondes\n", i + 1, logs[i].algo, logs[i].n, 130,logs[i].execution_time);
    }
}
    ...
    do {
        ...
        if (choix_algo == 1 || choix_algo == 2) {
            ...
            if (choix_algo == 1) {
                ...
                add_log(logs, "R\x82cursif", n, execution_time, &log_count);
            } else if (choix_algo == 2) {
                printf("Algorithme it%cratif :\n",130);
                hanoi_iteratif(n, 'A', 'C', 'B');
                end_time = clock();
                execution_time = (double)(end_time - start_time) / CLOCKS_PER_SEC;
                add_log(logs, "It\x82ratif", n, execution_time, &log_count);
            }
